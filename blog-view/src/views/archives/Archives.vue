...
<script>
	...
	import {isSuccess, normalizeBlogs} from "@/util/gvaResponse";
		...
		methods: {
			getArchives() {
				getArchives().then(res => {
					if (isSuccess(res)) {
						this.blogMap = Object.keys(res.data.blogMap || {}).reduce((result, key) => {
							result[key] = normalizeBlogs(res.data.blogMap[key])
							return result
						}, {})
